...
namespace HermaFx.Reflection
{
	public static class ReflectionHelper
	{
		...
		public static PropertyDescriptor GetPropertyDescriptor<TModel>(Expression<Func<TModel, object>> expression)
		{
			var pi = ReflectionHelper.GetProperty(expression);
			var descriptors = TypeDescriptor.GetProperties(pi.DeclaringType);

			return descriptors.Find(pi.Name, false);
		}
		...
		public static PropertyDescriptor GetPropertyDescriptor<TModel, T>(Expression<Func<TModel, T>> expression)
		{
			var pi = ReflectionHelper.GetProperty(expression);
			var descriptors = TypeDescriptor.GetProperties(pi.DeclaringType);
		...
		public static IEnumerable<FieldInfo> GetFields(object obj, BindingFlags relfectingFlags)
		{
			Type t = obj.GetType();
